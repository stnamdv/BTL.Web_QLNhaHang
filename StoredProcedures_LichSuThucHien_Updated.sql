/* =========================================================
   STORED PROCEDURES CHO QUẢN LÝ LỊCH SỬ THỰC HIỆN (UPDATED)
   Quản lý lịch sử thực hiện các bước xử lý đơn hàng
   Tham chiếu trực tiếp tới bảng Order thay vì OrderItem
   ========================================================= */
...
BEGIN
    ...
    JOIN dbo.[Order] o ON o.order_id = lsh.order_id;
...
-- =========================================================
-- 2. sp_LichSuThucHien_GetByOrder - Lấy lịch sử theo đơn hàng
-- =========================================================
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Validation: Kiểm tra order có tồn tại không
        ...
        IF EXISTS (SELECT 1 FROM dbo.LichSuThucHien WHERE order_id = @OrderId AND buoc_id = @BuocId)
        BEGIN
        ...
        -- Tạo lịch sử mới
        INSERT INTO dbo.LichSuThucHien (order_id, buoc_id, nv_id, trang_thai, ghi_chu)
        VALUES (@OrderId, @BuocId, @NvId, 'CHUA_BAT_DAU', @GhiChu);
...
BEGIN
    ...
    BEGIN TRY
        ...
        DECLARE @OrderId INT, @BuocId INT;
        SELECT @OrderId = order_id, @BuocId = buoc_id
        FROM dbo.LichSuThucHien
        WHERE lich_su_id = @LichSuId;
...
-- =========================================================
-- 8. sp_LichSuThucHien_GetTrangThai - Lấy trạng thái hiện tại của đơn hàng
-- =========================================================
